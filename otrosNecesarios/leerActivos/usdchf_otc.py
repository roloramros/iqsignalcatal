...
if velas_para_insertar:
    ...
    ejecutar_query(lambda: supabase.table("usdchf_otc").insert(velas_para_insertar).execute())
...
while True:
    candles = I_want_money.get_candles("USDCHF-OTC", 60, 2, time.time())
    if candles:
        ...
        if candle_time != last_candle_time:
            ...
            ejecutar_query(lambda: supabase.table("usdchf_otc").insert(data).execute())
