...
def verificar_integridad_velas():
    response = ejecutar_query(lambda: supabase.table("usdhkd_otc")
                              .select("*")
...
ejecutar_query(lambda: supabase.table("usdhkd_otc").delete().neq("id", 0).execute())
...
if velas_para_insertar:
    ...
    ejecutar_query(lambda: supabase.table("usdhkd_otc").insert(velas_para_insertar).execute())
...
while True:
    candles = I_want_money.get_candles("USDHKD-OTC", 60, 2, time.time())
    if candles:
        ...
        if candle_time != last_candle_time:
            ...
            ejecutar_query(lambda: supabase.table("usdhkd_otc").insert(data).execute())
            ...
            if ahora.hour == 3 and (ultima_eliminacion is None or ultima_eliminacion.date() < ahora.date()):
                ...
                ejecutar_query(lambda: supabase.table("usdhkd_otc")
                               .delete()
